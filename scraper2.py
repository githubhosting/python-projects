...
class SIS_Spider(scrapy.Spider):
    name = 'sis'
    login_url = 'http://parents.msrit.edu/parents_even2022/index.php'
    base_url = 'http://parents.msrit.edu/parents_even2022/'
    ...
    def parse(self, response, DOB=None):
        usn = "1MS21CI049"
        token = response.xpath('//input[@value="1"]/@name')[0].extract()
        password = ''
        for c in DOB:
            password = password + c + ''.join(
                random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(2))
    ...
    def attendance_data(self, response):
        ...
        # print(' p = ',att['teacher'])
        att['present'] = \
            response.xpath('//div[@class="progress-bar progress-bar-success"]/@title')[0].extract().split(':')[1]
        att['absent'] = \
            response.xpath('//div[@class="progress-bar progress-bar-danger"]/@title')[0].extract().split(':')[1]

        try:
            att['remaining'] = \
                response.xpath('//div[@class="progress-bar progress-bar-warning"]/@title')[0].extract().split(':')[1]
        except:
            att['remaining'] = '0'
