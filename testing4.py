...
sign_in = br.open("https://adminpannel.vercel.app/#/login")  # the login url

# br.select_form(nr=0)  # accessing form by their index. Since we have only one form in this example, nr =0.
br.select_form(
    name="username")  # Alternatively you may use this instead of the above line if your form has name attribute available.

br["username"] = "admin@foodey.com"  # the key "username" is the variable that takes the username/email value

br["password"] = "adminfoodey"  # the key "password" is the variable that takes the password value
